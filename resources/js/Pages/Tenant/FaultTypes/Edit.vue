...
const props = defineProps({
    tenant: Object,
    user: Object,
    faultType: Object,     // tipo de avería a editar
    priorities: Array,     // [{ value, label }]
})

const form = useForm({
    name: props.faultType.name ?? '',
    description: props.faultType.description ?? '',
    priority: props.faultType.priority ?? null,
})

const submit = () => {
    form.put(route('fault-types.update', props.faultType.id), {
        onSuccess: () => console.log('Tipo de avería actualizado'),
    })
}
</script>
